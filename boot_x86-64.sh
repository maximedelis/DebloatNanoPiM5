...
lodev_boot="$(sudo /usr/sbin/losetup -f)"
...
sudo dd if=uboot/idbloader of="${lodev_boot}" seek=64 bs=512 && sync
sudo dd if=uboot/u-boot.itb of="${lodev_boot}" seek=16384 bs=512 && sync
